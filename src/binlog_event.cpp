...
namespace dts
{
    int field_data_t::get_value_string(std::string &ret_res, bool b_is_encode) const
    {
        ...
        case BINLOGLIB_TYPE_BINARY:
            if (b_is_encode)
            {
                ...
                std::vector<uint8_t> data_vect(need_len + 1);
    ...
    int rows_log_event::unpack(const char *buf, const uint32_t len, format_description_log_event &desc_event, parser_info &info)
    {
        ...
        for (const char *value = row_data; value < row_data + data_len;)
        {
            ...
            row_data_t row_data;
            std::vector<field_data_t> &data_vect = ((event_type == WRITE_ROWS_EVENT) || (event_type == WRITE_ROWS_EVENT_V2)) ? row_data.new_data_vect : row_data.old_data_vect;
                ...
                event_type == UPDATE_ROWS_EVENT_V2)
            {
                ...
                */
                ...
                {
                    if (row_data.old_data_vect[i].is_monitor_change && !row_data.old_data_vect[i].value_equal(row_data.new_data_vect[i]))
                    {
        ...
        const table_info_t &table_info)
    {
        ...
        if (column_cnt != table_info.field_metadata_vect.size() || column_cnt != table_info.field_type_vect.size())
        {
