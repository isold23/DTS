...
int str2type(
    ...
    bool* is_signed)
{
    if(NULL == type_str || NULL == mysql_type
            || NULL == binloglib_type || NULL == is_signed) {
        return -1;
    }
    ...
    *mysql_type = MYSQL_TYPE_UNKNOW;
    ...
    /*}}}*/
        ...
        *is_signed = false;
        mysql_type2binloglib_type(binloglib_type, *mysql_type, *is_signed);
    } else if(0 == memcmp(type_str, "geometry", 9)
              || 0 == memcmp(type_str, "geometrycollection", 17)
              || 0 == memcmp(type_str, "multipoint", 11)
              || 0 == memcmp(type_str, "linestring", 11)
              || 0 == memcmp(type_str, "point", 5)
              || 0 == memcmp(type_str, "multilinestring", 16)
              || 0 == memcmp(type_str, "polygon", 8)
              || 0 == memcmp(type_str, "multipolygon", 13)) {
