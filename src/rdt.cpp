...
const std::string gstrProgramName = "dts";

static dts::binlog_worker worker;
...
void help_info(void)
{
    std::cout << "Real-time Data Transfer Server(dts) Help Infomation:" << std::endl;
    std::cout << "./dts -f configure_file_name(run application)" << std::endl;
    std::cout << "./dts -v(display server version info)" << std::endl;
    std::cout << "./dts -h(display help infomation)" << std::endl;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i = i + 2)
    {
        ...
        {
            std::cout << "Real-time Data Transfer Server(dts) Version Infomation:" << std::endl;
            std::cout << "dts Version: " << gstrProgramVersion.c_str() << " " << __DATE__ << " " << __TIME__ << std::endl;
            return 0;
        }
