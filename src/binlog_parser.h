...
private:
    ...
    inline int get_checksum_alg(const char* event_body, const char* event_pack, const uint32_t event_len)
    {
        ...
        checksum_alg = (version_product(version_split) < checksum_version_product) ?
                       (uint8_t) BINLOG_CHECKSUM_ALG_UNDEF :
                       *(uint8_t*)(event_pack + event_len - BINLOG_CHECKSUM_LEN - BINLOG_CHECKSUM_ALG_DESC_LEN);
        return 0;
    }
